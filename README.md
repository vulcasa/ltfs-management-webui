...
### v1.0.1 (2026-02-28)
- ✨ 磁带目录统合：多张磁带的目录结构统一显示在一个根目录下
- 🎨 目录树样式调整：统一缩进、添加加减号展开/收缩
- 📁 文件管理优化：在文件列表中新增"文件路径"列
- 🚀 时区配置：使用 Asia/Shanghai 时区
- 🧹 代码清理：移除虚拟磁带模拟器、删除临时文档
- 📦 版本配置：应用名称为 ltfs-management-webui，版本 1.0.1-beta
- 🔧 磁带状态修复：应用启动时重置所有磁带状态为 ejected
- 🐛 Bug 修复：当前磁带状态显示、条形码解析等问题修复

### v1.0.0-beta (2026-02-27)
- 完整的项目初始化
- Flask 应用基础架构
- 数据库模型设计
- 现代化 WebUI 界面
